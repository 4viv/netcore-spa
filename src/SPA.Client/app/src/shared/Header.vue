      ...
      <div class="container">
        ...
        <div id="navbarMenuHeroC" class="navbar-menu">
          <div class="navbar-end">
            <router-link :class="{'is-active': $route.name === 'default'}" class="navbar-item is-active" to="/">Inicio</router-link>
            ...
            <router-link :class="{'is-active': $route.path.startsWith('products')}" class="navbar-item" to="/products">Productos</router-link>
              <!-- Se muestra solo si incluye Admin  -->
...
export default {
  ...
  data() {
    return {
      user: this.$store.state.user // traemos info user
    }
  },
  methods: {
    logout() {
        //Borramos la secion de localStorage
      localStorage.removeItem("access_token");
      this.$parent.isLoggedIn = false; // modificamos la variable de App
    }
  }
};
</script>