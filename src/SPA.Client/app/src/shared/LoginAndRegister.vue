...
export default {
  ...
  data() {
    return {
      ...
      login: {
        email: null,
        password: null,
        loading: false // para bloquera el boton
      },
      register: {
        email: null,
        password: null,
  ...
  methods: {
    authenticate() {
      this.login.loading = true; //activa boton
      this.$proxies.identityProxy
        .login(this.login)
        .then(x => {
          this.login.loading = false;
          this.$parent.isLoggedIn = true; //modifica la propiedad del comp padre App
            //Notificacion
          this.$notify({
            group: "global",
            type: "is-success",
            ...
            //Guardamos el token em localStorage
          localStorage.setItem("access_token", x.data);
          this.$user.initialize(); // traemos info del tocken
        })
        .catch(x => {
          ...
          this.login.loading = false; // desactiva boton
        });
    },
    addNewUser() {
