...
export default {
  ...
  methods: {
    initialize() {
      let self = this; //variable para poder accesar this desde las funciones
      ...
      function __prepareConfig() {
        if (!localStorage.getItem("config")) {
          ...
          // Metodo para mantener viva la sesion
      function __isLoggedIn() {
        if (localStorage.getItem("access_token") != null) {
          self.$user.initialize(); // Trae info del tocken de mainjs
